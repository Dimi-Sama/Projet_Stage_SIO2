...
if(isset($_POST['saveStyle']))
{
    ...
    $repImg = '../../../asset/imgStyle/';

    if($name == NULL && $imgName == NULL)
    {
        $rep = [
            'status' => 0,
    ...
    $sql = "INSERT INTO style (id,libelle,img) VALUES (NULL, :libelle, :img)";
...
if(isset($_POST['updateStyle']))
{
    ...
    $sql = "UPDATE style SET libelle= :libelle, img = :img WHERE id= :id";
    $req = $conn->prepare($sql);
...
if(isset($_POST['updateLimite']))
{
    ...
    $sql = "UPDATE limite SET limStyle= :limite";
    $req = $conn->prepare($sql);
